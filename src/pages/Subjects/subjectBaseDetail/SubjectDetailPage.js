...
import { FiEdit, FiTrash2, FiArrowLeft, FiBook } from "react-icons/fi";
...
import SubjectStudentsModal from "../SubjectStudentsModal/SubjectStudentsModal";
...
const SubjectDetailPage = () => {
  ...
  const [topicForAssign, setTopicForAssign] = useState(null);
  ...
  const closeStudentsModal = () => {
    ...
    setTopicForAssign(null);
  };

  const handleAssignedStudents = async () => {
  ...
  return (
    <div className={styles.container}>
      <PageHeader
        title={
          subject
            ? `Fənn: ${subject.subjectBase?.name || subject.name}`
            : "Fənn detalları"
        }
      />
      ...
      <SubjectStudentsModal
        ...
        subject={topicForAssign}
        onClose={closeStudentsModal}
        onAssigned={handleAssignedStudents}
      />
    </div>
  );
};
